...
function DropPt( x,y,w,h,id,max,f ) {
    this.x = x
    this.y = y
	this.w = w
	this.h = h
    this.dropId = id
    this.maxDrops = max //maximum drop items per zone
    this.dragItem = null
    this.itemsInZone = [] //array that holds items currently in the drop zone
    this.updFunc = f
...
function removeItem(name){
    for(var i = 0; i < this.itemsInZone.length; i++) {
        if(this.itemsInZone[i].name == name) {
            this.itemsInZone.splice(i,1);
            return;
        }
    }
}
...
function DragItemRestore(func) {
	if ( !func ||  func == this.lyr.updateDragFunc ) 
	{
		...
		if( this.lyr.dropObj ) 
		{
		  this.lyr.dropObj.removeItem(this.name);
...
function AddDropPt(x,y,w,h,id,max,f) {
	
    var idx = this.arrDropPts.length
    this.arrDropPts[idx] = new DropPt( x,y,w,h,id,max,f )
}
function Reset() {
    ...
    for (var i=this.arrDragItems.length-1;i>=0;i--) 
    {
      ...
      if (dItem.checkWithinItem(x,y)) 
      {
        ...
        if( this.item.lyr.dropObj ) 
        {
          ...
          this.item.lyr.dropObj.removeItem(dItem.name);
...
function DragCheckDropPts() {
    for( var pass = 0; pass < 2; pass++ ) 
    {
        for (var i = this.arrDropPts.length-1;i>=0;i--) 
        {
            ...
            {
                var bDropZoneFull = dropPt.itemsInZone.length >= dropPt.maxDrops;
                if(bDropZoneFull){
                    trivAlert(dropPt.dropId,VarCurrentTitleName.getValue(),trivstrDDW.replace("##0",dropPt.maxDrops));
                    return false;
                }
                dropPt.dragItem = this.item
                dropPt.itemsInZone.push(this.item);
...
function dragMouseDown(e) {
    ...
    if ((is.ns && e.which!=1) || (is.ie && getDisplayWindow().event.button!=1)) return true
    var A = getAdjXY(e);
...
function getAdjXY(e) 
{
   if(getDisplayWindow().bScaleToWindow){
        var theWin = getDisplayWindow(),
        doc = getDisplayDocument(),
        pagePlayer = theWin.pagePlayer,
        curPage = pagePlayer ? pagePlayer.activePage : null,
        touchEvent = (!!theWin.TouchEvent && e instanceof TouchEvent),
        valx = (touchEvent|| (isSinglePagePlayerAvail() && e.changedTouches)) ? e.changedTouches[0].pageX : (e.pageX ? e.pageX : (e.x + doc.body.scrollLeft)),
        valy = (touchEvent|| (isSinglePagePlayerAvail() && e.changedTouches)) ? e.changedTouches[0].pageY : (e.pageY ? e.pageY : (e.y + doc.body.scrollTop)),
        topdiv = curPage ? curPage.div : doc.getElementById('pageDIV'),
        offset = triv$(topdiv).offset();

        if(offset)
        {
            valx -= Math.round(offset.left);
            valy -= Math.round(offset.top);
        }
